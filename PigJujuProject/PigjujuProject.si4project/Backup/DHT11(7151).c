...
{	
    ...
    while (1)
    {
        ...
        if (readSensorData())
        {
           ...
           temp = (databuf >> 8) & 0xff  + (databuf & 0xff)/10 ;
           //printf("temp=%d\n",temp);	

		   Maindata->DHT11_humidity = humi;
		   Maindata->DHT11_temp = temp;
           // strncmp(&Maindata->DHT11_humidity , (databuf >> 24) & 0xff , sizeof(databuf));
            //printf("Rh=%d\n",Maindata->DHT11_humidity);

            //Maindata->DHT11_temp = ((databuf >> 8) & 0xff)  ;//+  (databuf & 0xff)
            //printf("temp=%d\n",&Maindata->DHT11_temp);
#endif
